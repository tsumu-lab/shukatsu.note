...
export default function PersonalMemoCard({
  ...
  if (isEditing) {
    return (
      <form ref={formRef} action={updatePersonalMemo} onBlur={handleBlur} className="space-y-1">
  ...
  return (
    <div className="flex items-start gap-2">
      ...
      <PinButton pinned={note.pinned} formData={{ id: note.id, category: "memo" }} action={togglePinNote} className="pt-3" />
    </div>
  );
}